...
export const WordsPage: React.FC = () => {
  ...
  const changeLevel = (e: MouseEvent): void => {
    ...
    //   getWords(Number(target.dataset.group), MagicNumbers.ZER0_VALUE).then(
  ...
  return (
    <div className="words-page">
      {/* <nav className="words-navigation">
        <button onClick={() => dispatch(unLoadWordsAction())} className="words-home">
          {Text.closeBookBtn}
        </button>
        <div className="buttons-page_wrapper">
          <button
            onClick={(e) => changePage(e)}
            className="words-prev-page words-btn_default"
            data-prev="prev"
            disabled={state.page === MagicNumbers.ZER0_VALUE ? true : false}
          >
            {Text.authorizationBackBtn}
          </button>
          <div className="select">
            <div
              ...
              dangerouslySetInnerHTML={{ __html: state.selected }}
            ></div>
            {state.isActive && (
              <ul className="select-options">
                {levelArr.map((item: string) => (
                  <li
                    className="option"
                    data-group={levelArr.indexOf(item)}
                    key={item}
          ...
          <button
            onClick={(e) => changePage(e)}
            className="words-next-page words-btn_default"
            data-next="next"
            disabled={state.page === MagicNumbers.MAX_BOOK_PAGE ? true : false}
          >
            {Text.nextBookPage}
          </button>
        </div>
          ...
          <div className="words-btn_wrapper">
            {state.allWords.map((item: WordsType) =>
              state.allWords?.indexOf(item) === MagicNumbers.ZER0_VALUE ? (
                <button
                  onClick={(e) => activeWord(e)}
                  className="word-wrapper active-word"
                  data-key={item.id}
                  key={item.id}
                  data-page={item.page}
